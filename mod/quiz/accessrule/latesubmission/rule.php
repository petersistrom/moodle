 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class quizaccess_latesubmission extends quiz_access_rule_base {
    private const OVERDUE = "Yes";
    private const ONTIME = "No";

    // Grace period to check if
    ...
    public static function add_settings_form_fields(mod_quiz_mod_form $quizform, MoodleQuickForm $mform) {
        ...
        $appliedpenalty = $mform->createElement('selectyesno', 'appliedpenalty', get_string('appliedpenalty', 'quizaccess_latesubmission'));
        ...
        $dailypercentage = $mform->createElement('text', 'dailypercentagepenalty', get_string('dailypercentagepenalty', 'quizaccess_latesubmission'));
        ...
        $preventresubmission = $mform->createElement('selectyesno', 'preventresubmission', get_string('preventresubmission', 'quizaccess_latesubmission'));
    ...
    public static function save_settings($quiz) {
        ...
        if (empty($record)) {
            ...
            $record->preventresubmission = $quiz->preventresubmission;
     ...
     * here is probably to read the code of {@link quiz_access_manager::load_settings()}.
     *
    ...
    public static function get_settings_sql($quizid) {
        return array(
            'timedue, appliedpenalty, dailypercentagepenalty, preventresubmission',
    ...
    public static function calculate_percentage_penalty($attempt) {
        ...
        if (!empty($lateattempt->appliedpenalty) && ($timedue <= $lateattempt->timefinish)) {
            $timeoverdue = $lateattempt->timefinish - quizaccess_latesubmission::GRACE_PERIOD - $lateattempt->timedue;
    ...
    public static function update_overdue_status($attempt) {
        ...
        if (!empty($quiz)) {
            $timedue = $quiz->timedue ? $quiz->timedue + quizaccess_latesubmission::GRACE_PERIOD : 0;
