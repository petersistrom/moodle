 ...
 */
class quizaccess_latesubmission_testcase extends advanced_testcase {
    ...
    private function create_quiz_object($course, $quiz,
                                        ...
                                        $dailypercentagepenalty = 0, $preventresubmission = false ) {
    ...
    public function test_latesubmission_description() {
        ...
        $this->assertContains(get_string('quizdueon', 'quizaccess_latesubmission', userdate($timeduesetting)), $rule->description());
        $this->assertContains(get_string('quizduewithoutpenalty', 'quizaccess_latesubmission', userdate($timeduesetting)), $rule->description());

        // Due on a future time with penalty of 5 percent.
        $rule = new quizaccess_latesubmission($this->create_quiz_object($course, $quiz, $timeduesetting, true, 5), 3659);
        $this->assertContains(get_string('quizdueon', 'quizaccess_latesubmission', userdate($timeduesetting)), $rule->description());
        $this->assertContains(get_string('quizduewithpenalty', 'quizaccess_latesubmission', 5), $rule->description());

        // Already due without penalty.
        $rule = new quizaccess_latesubmission($this->create_quiz_object($course, $quiz, $timeduesetting), 3660);
        $this->assertContains(get_string('quizalreadydue', 'quizaccess_latesubmission', userdate($timeduesetting)), $rule->description());
        $this->assertContains(get_string('quizduewithoutpenalty', 'quizaccess_latesubmission', userdate($timeduesetting)), $rule->description());
    ...
    public function test_no_penalty() {
        ...
        $this->assertEquals(0, quiz_access_manager::accumulate_percentage_penalty($attempt));
        // Check grade.
    ...
    public function test_calculate_percentage_penalty($timefinish, $expectedpenalty, $expectedgrade, $regrade = -1, $late = true) {
        ...
        $this->assertEquals($expectedpenalty, quiz_access_manager::accumulate_percentage_penalty($attempt));
        // Identify as late.
        $record = $DB->get_record('quizaccess_latesubmission_at', ['attemptid' => $attempt->id]);
        if ($late) {
